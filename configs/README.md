...
sample count, mask rendering, degradation preset selection, and worker count. `workers: auto` is
the default and uses every CPU assigned to the process (including a Docker CPU limit) for
independent LilyPond render tasks and degradation/materialization tasks. Generation has no CUDA
kernel path: LilyPond, Pillow, and NumPy execute on CPU, so the GPU stays available for training.
...
`evaluation/full_run_gpu.yaml` is the container-mount-path companion used only by
`scripts/release/full_run_gpu.sh`. It evaluates the three checkpoints generated by that full CUDA
workflow and explicitly requires CUDA for evaluation and benchmarks.
